-- 001 initial schema
--
-- The schema as it stood when the sandbox shipped, with two changes:
--   * no DROP SCHEMA. Migrations are forward-only and never destroy data.
--   * no CREATE ROLE. Roles are cluster-level objects, not schema objects, and
--     their passwords come from the environment. See db/bootstrap.js.
--
-- Buyer isolation is enforced here, in the database, not in application code.
-- The application connects as the runtime role, which is NOT the table owner
-- and NOT a superuser, so row-level security cannot be bypassed by a bad route.

CREATE SCHEMA IF NOT EXISTS plint;
...
-- ------------------------------------------------------------- the app role
-- The role itself is created by db/bootstrap.js with a password from the
-- environment. This migration only grants it what it may do.
